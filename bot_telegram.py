...
🔗 <a href="{annonce.link}">Voir l'annonce</a>
...
def envoyer_message(PC, annonce):
    ...
    if annonce.image and len(texte) <= 1024:
        url = f"https://api.telegram.org/bot{TOKEN}/sendPhoto"
        params = {"chat_id": CHAT_ID, "photo": annonce.image, "caption": texte, "parse_mode": "HTML"}
    elif annonce.image:
        # Image + texte trop long pour une caption -> envoie les deux séparément
        photo_url = f"https://api.telegram.org/bot{TOKEN}/sendPhoto"
        requests.post(photo_url, data={"chat_id": CHAT_ID, "photo": annonce.image})
    ...
    if not response.json().get("ok"):
        print("Erreur envoi Telegram :", response.json())
